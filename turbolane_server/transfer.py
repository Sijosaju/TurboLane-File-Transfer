...
class StreamWorker:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            # LAN optimization: increase TCP send/receive buffers to 4 MB per stream
        ...
        try:
            with open(self._cq.file_path, "rb") as fh:
                while not self._stop_event.is_set() and not self._cq.is_done:
                    ...
                    if not success:
                        # BUG FIX: always requeue unacked chunks, even when
                        # stop_event is set (e.g. adapter removed this stream).
                        # Without this, chunks grabbed from the queue but not
                        # yet ACKed are silently dropped when a stream is torn
                        # down, causing the transfer to stall near 100%.
                        logger.debug(
